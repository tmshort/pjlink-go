...
func (request *Request) validateCommandParameter() error {
	if request.Class == 1 {
...
func (request *Request) toRaw(seed string, password string) string {
	if seed == "" || password == "" {
...
// returns string hash
func (request *Request) createEncryptedMessage(seed, password string) string {
	//generate MD5
